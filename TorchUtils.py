...
class ImageDataset(Dataset): # I think this should be good now =========
    def __init__(self,X,Y): # X and Y must be np.array() types
...
class ScanDataModule(pl.LightningDataModule): # NEEDS FIXING
    ...
    def train_dataloader(self):
        train_dataloader = DataLoader(self.train_set,batch_size=self.batch_size,shuffle=True, num_workers=self.num_workers)  # input:(512,512), label:1
        return train_dataloader
    
    def val_dataloader(self):
        val_dataloader = DataLoader(self.valid_set,batch_size=self.batch_size,shuffle=False, num_workers=self.num_workers)  # input:(512,512), label:1
        return val_dataloader
    
    def test_dataloader(self):
        test_dataloader = DataLoader(self.test_set,batch_size=self.batch_size,shuffle=False, num_workers=self.num_workers)  # input:(512,512), label:1
